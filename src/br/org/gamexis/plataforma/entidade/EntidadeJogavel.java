...
public class EntidadeJogavel extends EntidadeAnimada implements
	...
	private float aceleracaoCorrendo = 4;
	private float aceleracaoNoAr = 2;
...
//			} else
			...
			if (coli.getFonte().getAtor().getCorpo().getPosition()
	...
	public void aplicarDirecionalNoAr(Ator jogavel) {
		...
		if (direitaApertado) {
			...
			if (jogavel.getCorpo().getVelocity().getX() <= 16f) {
				jogavel.getCorpo().adjustVelocity(new Vector2f(aceleracaoNoAr, 0));
			}
...
//		Ator jogavel = (Ator) getAtor();
		...
		if(pularApertado) {
			...
			long atual = (Sys.getTime() * 1000) / Sys.getTimerResolution();
			
			if(Math.abs(atual - inicioPular) > 80 ) {
				impulsoPulo = -40f;
				acaoPular();
				pularApertado = false;
			} else {
	...
	private void atualizarPulo() {
		if(Math.abs(impulsoPulo) < 40 )
			impulsoPulo = impulsoPulo - 0.5f;
			...
			if (direitaApertado) {// ANDAR
				...
				if (correrApertado) {
					...
					if (jogavel.getCorpo().getVelocity().getX() <= 32f) {
						jogavel.getCorpo().adjustVelocity(new Vector2f(aceleracaoCorrendo, 0));
					} else {
						jogavel.zerarVelocidadeX();
						jogavel.setVelocidadeX(32);
					}
				} else {
					...
					if (jogavel.getCorpo().getVelocity().getX() <= 16f) {
						jogavel.getCorpo().adjustVelocity(new Vector2f(aceleracao, 0));
					} else {
						jogavel.zerarVelocidadeX();
						jogavel.setVelocidadeX(16);
					}
				}
	...
	public void atualizarFisica(int delta) {
		...
		if(jogavel.isNoar() || jogavel.isCaindo() || jogavel.isVoando()) {
	...
	public void reiniciar() {
		...
		armas.desabilitarTodas();
		armas.habilitar("EV");
		arma = armas.get("EV");
		painelEstado.setArma(arma.getNome());
		arma.configureModoNormal();
		acaoProximaArma();
	...
	private  void habilitarPulo() {
		if (!((Ator) getAtor()).isNoar() && !((Ator) getAtor()).isCaindo() && !((Ator) getAtor()).isPulando() && !pularApertado) {
			pularHabilitado = true;
		}
	}
	...
	private boolean iniciarDoContadorPulo = false;
	...
	private void controlePressionado(int chave) {
		if(chave == Controles.T_S) {
			...
			if(!pularApertado && pularHabilitado) {
				...
				if(!iniciarDoContadorPulo) {
					inicioPular = (Sys.getTime() * 1000) / Sys.getTimerResolution();
					iniciarDoContadorPulo = true;
					impulsoPulo = -10;
				}
