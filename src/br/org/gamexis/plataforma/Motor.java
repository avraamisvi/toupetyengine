...
import org.neodatis.odb.core.query.criteria.EqualCriterion;
...
public class Motor extends BasicGame {
	...
	private static final String RECURSOS_IMAGENS_FADE = "fade.png";
	 ...
	 * <b>B</b> - Beta
	 */
	private String versao = "1.0.0.Beta build 242";
...
//		mot.gerarConfiguracaoPadrao();
	...
	//TODO reajustar todas as configuracoes
	...
	public void initPadrao(GameContainer container) throws SlickException {
		...
		if(cenaInicialDebug != null) {
			cenaAtual = new CarregadorCenario().carregarCena(getCenaInicialDebug());
		} else {
...
//		EntradaPerifericos.getInstancia().getControle().selecinarTeclado();
//		EntradaPerifericos.getInstancia().getControle().padrao();
//		container.getInput().disableKeyRepeat();
//		container.getInput().addPrimaryListener(getJogavelCarregado());
	}
	...
	public void update(GameContainer container, int delta)
		...
		if (container.getInput().isKeyPressed(Input.KEY_3)) {// SHOW FPS
			...
			if (modoDesenvolvedor) {
				...
				jogavelCarregado.setFogueteCargaMaxima(jogavelCarregado.getFogueteCargaMaxima()+100);
			}
		}
	...
	public void tratarExcecao(Throwable cause) {
		...
		Logger.getLogger(Motor.ARQUIVO_LOG).log(Level.SEVERE, "erro motor:", cause.getStackTrace());
	}
	...
	public ConfiguracaoGeral carregarConfiguracao() {
		...
		try {
			ODB odb = ODBFactory.open(ARQUIVO_CONFIGURACAO);
			IQuery query = new CriteriaQuery(ConfiguracaoGeral.class, new EqualCriterion("id", 0));
