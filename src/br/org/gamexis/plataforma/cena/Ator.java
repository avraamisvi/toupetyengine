...
public class Ator implements Desenho, Colisivel, Atualizavel {
	...
	// Embora exista a caixa de colisão não é avaliada pelo motor, ficando a cargo do 
	// designer codificar o seu comportamento no script
	private List<Rectangle> caixasColisao = new ArrayList<Rectangle>();
	...
	public Ator(Ator copia) {
		try {
			...
			for (String chave : copia.animacoes.keySet()) {
				adicioneAnimacao(chave, (Animado) copia.animacoes.get(chave).clone());
			}
...
//		if(corpo.getVelocity().getY() > 50) {
//			System.out.print("VY::::::" + corpo.getVelocity().getY());
//			System.out.println(" MaxVY::::::" + corpo.getMaxVelocY());
//		}
	}
	...
	public void desenhar(Graphics g) {
		...
		if (Motor.obterInstancia().isModoDebug()) {
			...
			g.drawString("Tipo entidade:" + getEntidade().getTipo().toString(), x, y-30);
	...
	/**
	 * Simula caixas de colisão, para permitir analisar se ator esta
	 * tocando caixa de colisão virtual.
	 * 
	 * @param comprimento
	 * @param altura
	 * @param x
	...
	public void finalize() throws Throwable {
		System.out.println("FINALIZADO(Ator):" + getNome());
		super.finalize();
	}
	 ...
	 * @param comprimento
	 * @param altura
	 * @param x deslocamento positivo em x(ou seja posicao x do jogavel + x de deslocamento)
	 * @param y deslocamento positivo em y
	 */
	public void adicionarCaixaColisao(float comprimento, float altura, float x, float y) {
		caixasColisao.add(new Rectangle(x, y, comprimento, altura));
	}
	
	public void setCaixasColisao(List<Rectangle> caixasColisao) {
		this.caixasColisao = caixasColisao;
	}
	 ...
	 * @param ator
	 * @
	 * @return retorna o false caso não haja colisão true caso contrario.
	 */
	public boolean colideCaixaColisao(Ator ator, int indice) {
		Rectangle c = caixasColisao.get(indice);
		
		return (estaTocando(c.getWidth(), c.getHeight(), getX() + c.getX(), getY() + c.getY()));
	}
