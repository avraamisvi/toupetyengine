...
public class CarregadorMosaicoAnimado {
	...
	class CarregadorXMLHandler extends DefaultHandler {
		...
		private MemoriaValorCusto recursos = RecursosFactory.getInstancia();
		...
		public void startElement(String uri, String localName, String tag,
			...
			if (tag.toUpperCase().equals("MOSAICOANIMADO")) {
				...
				try {
					if(!multi) {
						mosaico = RecursosFactory.getInstancia().getMosaicoAnimado(nome, base, comprimentoCelulas, alturaCelulas,
								linhas, colunas);
					} else {
						mosaico = RecursosFactory.getInstancia().getMultiImagemMosaicoAnimado(nome, base, comprimentoCelulas, 
