...
public class MosaicoAnimado implements Desenho, Animado {
	 ...
	 * @throws SlickException
	 */	
	public MosaicoAnimado(MultImagem tiles, int comprimento, int altura, int linhas,
			int colunas) throws SlickException {
	...
	@Override
	public void dispose() {
		//TODO Ver questao dos tiles
		for(Animado ani : animacoes) {
			((Animacao)ani).dispose();
		}
		
		if(imagens != null) {
			RecursosFactory.getInstancia().libertarRecurso(imagens.getReferencia());
			RecursosFactory.getInstancia().libertarRecurso(RecursosFactory._MOSAICO_ANIM_ + imagens.getReferencia());
		}
		
		if(spriteSheetBase != null) {
			RecursosFactory.getInstancia().libertarRecurso(RecursosFactory._MOSAICO_ANIM_ + spriteSheetBase.getReference());
		}
	...
	public void desenhar(Graphics g, float fatorX, float fatorY) {
		// TODO mosaico animado resize ainda nao suportado
		desenhar(g);
	}
}
