...
public class Controles {
	...
	public void setComandoTeclado(int comando, int correspondente) {
		...
		if(!teclado.isEmpty() && teclado.get(comando) != null) {
			int correspondenteAtual = teclado.get(comando);
			
			//remove o comando que deseja alterar
			fromTeclado.remove(correspondenteAtual);
			//remove o comando que deseja alterar
			teclado.remove(comando);
			
			//ja tem alguem usando
			if(fromTeclado.get(correspondente) != null) {
				//atualiza o esse alguem para a de quem quer trocar com ele
				teclado.put(fromTeclado.get(correspondente), correspondenteAtual);
			}
		} 
		teclado.put(comando, correspondente);
		fromTeclado.put(correspondente, comando);
	}
	
	public void setComandoJoystick(int comando, int correspondente) {
		
		if(fromJoystick.containsKey(correspondente)){
			joystick.put(fromJoystick.get(correspondente), joystick.get(comando)); 
		}
		
		joystick.put(comando, correspondente);
		atualizarJoystick();
	}
	...
	private void atualizarJoystick() {
		fromJoystick = new HashMap<Integer, Integer>();
		for(Integer chave : joystick.keySet()) {
			fromJoystick.put(joystick.get(chave), chave);
		}
	}
