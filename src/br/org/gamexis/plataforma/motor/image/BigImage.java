package br.org.gamexis.plataforma.motor.image;
...
/**
 * Copiado da biblioteca slick, para permitir o carregamento dos dados de imagens grandes 
 * a partir de um arquivo DAT.
 * @author abraao
 *
 */
public class BigImage extends Image {
	...
	public BigImage(InputStream in, String ref, int filter) throws SlickException {
		
		build(in, ref, filter, getMaxSingleImageSize());
	}
	...
	public BigImage(InputStream in, String ref, int filter, int tileSize) throws SlickException {
		build(in, ref, filter, tileSize);
	}	


	private void build(InputStream in, String ref, int filter, int tileSize) throws SlickException {
		try {
			final LoadableImageData data = ImageDataFactory.getImageDataFor(ref);
			final ByteBuffer imageBuffer = data.loadImage(in, false, null);
