...
//		frente.dispose();
	...
	public void atualizar(GameContainer container, int delta)
		...
		Desenho desenhos[] = todos.toArray(new Desenho[0]);
		
		for (Desenho desen : desenhos) {
			if (desen.excluido()) {
//				removaDesenho(desen);
...
//			purgeExcluidos();
	...
	//Criado para dar suporte a maquinas com implementaçao antiga de opengl
	public void limparFundo(Graphics g) {
		Color old = g.getColor(); 
		g.setColor(corFundo);
		float comp = Motor.obterInstancia().getConfigGeral().getResolucao().getComprimento();
		float alt = Motor.obterInstancia().getConfigGeral().getResolucao().getAltura();
	...
	//TODO Essa logica de exclusao nao esta otimizada.
	/* (non-Javadoc)
