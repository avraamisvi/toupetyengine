	...
	class CarregadorXMLHandler extends DefaultHandler {		
		...
		public void startElement(String uri, String localName, String tag,
			...
			if (tag.equalsIgnoreCase("CENARIO")) {
				...
				String descricao = attributes.getValue("descricao");
...
//									+ referencia + ".xml"));
				...
				try {
					fatorTamanhoX = Float.parseFloat(attributes.getValue("fatorTamanhoX"));
					fatorTamanhoY = Float.parseFloat(attributes.getValue("fatorTamanhoY"));
				} catch(Exception e) {
					fatorTamanhoX = 1;
					fatorTamanhoY = 1;
				}
				...
				try {
					...
					imagem.setX(x);
					imagem.setY(y);
					imagem.setFatorTamanhoX(fatorTamanhoX);
					imagem.setFatorTamanhoY(fatorTamanhoY);
