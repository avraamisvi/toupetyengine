		...
		public void startElement(String uri, String localName, String tag,
			...
			if (tag.toUpperCase().equals("ENTIDADE")) {
				...
				if(comportamento != null) {
					ScriptComportamento script = RecursosFactory.getInstancia().getComportamento(comportamento);
				...
				Ator ator;
				String animacao = attributes.getValue("animacao").toUpperCase();
...
//					SuperCorpo corpo = new SuperCorpo(cpAtor.getShape(), cpAtor.getMass());
//					corpo.setPosition(cpAtor.getPosition().getX(),
//							cpAtor.getPosition().getY());
					
					SuperCorpo corpo = new SuperCorpo(new Circle(ator.getCorpo()
							.getShape().getBounds().getWidth() / FATOR_DIAMETRO_JOGAVEL), cpAtor.getMass());
					corpo.setPosition(cpAtor.getPosition().getX(),
							cpAtor.getPosition().getY());					
					corpo.setFriction(cpAtor.getFriction());
					...
					/*Body circulo = new Body("Circulo", new Circle(ator.getCorpo()
							.getShape().getBounds().getWidth() / FATOR_DIAMETRO_JOGAVEL), 
							ator.getCorpo().getMass());
					
					circulo.setRotatable(false);
					circulo.setPosition(ator.getCorpo().getPosition().getX(),
							ator.getCorpo().getPosition().getY());
					circulo.setFriction(0.6f);
//					circulo.addExcludedBody(ator.getCorpo());					
					circulo.setPrivateData(ator);
					circulo.addExclusionKey(TipoEntidade.plataforma);

					// circulo.addExclusionKey( NivelCena.plataforma);
					corpo.addCorpoFixedJoint(circulo, true);*/
